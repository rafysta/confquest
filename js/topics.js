...
const TOPIC_DECK = [
  ...
  /* ============ v1.33.0 追加 — ISSY39(酵母の国際学会)と
   * Chung-Ang大訪問で実際に使える話題。学会の中と、その周辺の街の話。 ============ */
  { id: 'ac-pombe-beer', cat: 'humor', icon: '🍺', title: 'pombe はビール',
    opener: "Here is a fact for the banquet table: pombe, as in Schizosaccharomyces pombe, is Swahili for beer. It was isolated from East African millet beer in 1893.",
    openerJa: '懇親会用の小ネタをひとつ。pombe(分裂酵母の種名)はスワヒリ語で「ビール」です。1893年に東アフリカのキビの酒から分離されました。',
    follow: ["So technically I have spent my whole career studying beer. My family finds this very reassuring.",
             "Does your organism have a story behind its name?"],
    followJa: ['つまり私は人生をかけてビールを研究してきたわけです。家族はこれを聞いて安心しています。', 'あなたの扱っている生物の名前には、何か由来がありますか?'],
    note: '酵母の学会で最強の掴み。事実+自虐+相手への振り、の三段で作ってある。グラスを持っているときに出すと必ず受ける。' },
  { id: 'ac-single-track', cat: 'academia', icon: '🪑', title: '単一トラックの学会',
    opener: "I like single-track meetings. You cannot escape, so by day three you actually know the person sitting next to you.",
    openerJa: '単一トラックの学会は好きです。逃げ場がないので、3日目には隣の席の人と本当に知り合いになっている。',
    follow: ["Was that a deliberate choice by the organizers, or just the size of the venue?",
             "Do you prefer big parallel meetings or small ones like this?"],
    followJa: ['それは運営の意図的な選択ですか?それとも会場の広さの都合?', '大きな並行セッションの学会と、こういう小さい会、どちらが好きですか?'],
    note: '会議そのものを褒める話題は、主催側の人に対して最も安全で嬉しい。相手が運営でなくても「学会観」の話に広がる。' },
  { id: 'ac-invited-nerves', cat: 'academia', icon: '🎤', title: '招待講演の裏側',
    opener: "People assume invited speakers are relaxed. I rewrote my first three slides at 1 a.m. last night.",
    openerJa: '招待講演者は余裕があると思われがちですが、私は昨夜1時に最初の3枚を書き直していました。',
    follow: ["Do you still get nervous before a talk, after all these years?",
             "What is your rule for the first slide?"],
    followJa: ['これだけ経験を積んでも、発表前はまだ緊張しますか?', '最初の1枚について、何かルールを持っていますか?'],
    note: '弱さを先に見せると、相手も本音を出しやすくなる。「最初の1枚のルール」は、どんなベテランでも語りたい話題。' },
  { id: 'ac-nonmodel', cat: 'academia', icon: '🧫', title: '非モデル生物のおもしろさ',
    opener: "Half the talks here are about yeasts I had never heard of. It is humbling — I have spent twenty years on one species.",
    openerJa: 'ここでの発表の半分は、名前も知らなかった酵母の話です。謙虚になります — 私は20年間ひとつの種だけを見てきたので。',
    follow: ["If you had to start over with a different organism, which one would you pick?",
             "What can a non-model species do that the model ones cannot?"],
    followJa: ['もし別の生物で研究をやり直すなら、どれを選びますか?', 'モデル生物にできなくて、非モデル種にできることは何ですか?'],
    note: '相手の生物に敬意を示す入り方。「やり直すならどれ?」は相手が最も楽しく答えられる仮定の質問。' },
  { id: 'ac-own-software', cat: 'academia', icon: '💻', title: '自分で道具を作る話',
    opener: "I ended up writing my own browser for our data, mostly because I got tired of screenshots. It is on GitHub now.",
    openerJa: '結局、自分のデータ用のブラウザを自分で書きました。スクリーンショットに疲れたのが理由です。今はGitHubに置いてあります。',
    follow: ["Does your lab build its own tools, or do you stick to published pipelines?",
             "Do you let students spend time on software? I still argue with myself about that."],
    followJa: ['あなたの研究室は自分で道具を作りますか?それとも公開されたパイプラインを使いますか?', '学生にソフト開発の時間を使わせますか?私はいまだに自問しています。'],
    note: '「作った自慢」ではなく「疲れたから作った」という動機で話すと嫌味がない。研究室運営の話に広げられる二段構え。' },
  { id: 'ac-send-students', cat: 'academia', icon: '✈️', title: '学生を海外に出す',
    opener: "I keep telling my students to apply abroad, and then I remember that four out of five labs never replied to my own emails.",
    openerJa: '学生には海外に応募しろと言い続けているのですが、自分のときは5つ書いて4つは返事が来なかったことを思い出します。',
    follow: ["How many of your students go abroad these days? Is it getting harder?",
             "What do you actually say to a student who is afraid to apply?"],
    followJa: ['最近、あなたの学生はどのくらい海外に出ますか?難しくなっていますか?', '応募を怖がっている学生に、実際どう言いますか?'],
    note: '自分の失敗を数字で出すのがこのカードの要。教員同士なら必ず自分の話をしてくれるし、学生が同席していれば最高の教材になる。' },
  { id: 'kr-old-town', cat: 'korea', icon: '🏛️', title: '会場のまわりの旧市街',
    opener: "I walked around the venue this morning — old stone walls on one side, glass towers on the other. Seoul does that contrast very well.",
    openerJa: '今朝、会場のまわりを歩きました。片側に古い石垣、反対側にガラスの高層ビル。ソウルはこの対比が上手ですね。',
    follow: ["Is this neighbourhood historically important? I felt like I was missing the story.",
             "Where would you take a visitor who only has two hours?"],
    followJa: ['この一帯は歴史的に重要な場所ですか?背景を知らずに歩いている気がして。', '2時間しかない訪問者を、どこへ連れて行きますか?'],
    note: '「歩いた」という事実から入ると観光ガイド的にならない。2時間という制限をつけると、相手が本気で一つ選んでくれる。' },
  { id: 'kr-banquet-table', cat: 'korea', icon: '🥢', title: '韓定食のテーブルで',
    opener: "There are more small dishes on this table than I can count. Which one should I start with, and which one will defeat me?",
    openerJa: 'このテーブル、小皿が数えきれません。どれから手をつけるべきで、どれが私を打ち負かしますか?',
    follow: ["Is there an order you are supposed to follow, or is it a free-for-all?",
             "What is the one dish foreigners always avoid but should not?"],
    followJa: ['食べる順番の決まりはありますか?それとも自由?', '外国人がいつも避けるけれど、本当は避けるべきでない料理は?'],
    note: '食卓では「教えてもらう立場」に立つのが一番強い。自虐(打ち負かされる)を入れておくと、辛い料理を勧められても逃げ道がある。' },
  { id: 'kr-cafe', cat: 'korea', icon: '☕', title: 'カフェの多さ',
    opener: "I have never seen so many cafés in one street. Is this a Seoul thing, or is the whole country like this?",
    openerJa: 'ひとつの通りにこんなにカフェがあるのは初めて見ました。ソウルだけですか?それとも国全体?',
    follow: ["Do people actually work in cafés here, or is it more for meeting friends?",
             "Where do you go when you need to think? I do my best thinking away from my desk."],
    followJa: ['ここでは実際にカフェで仕事をするんですか?それとも友人と会う場所?', '考えごとをしたいとき、どこに行きますか?私は机から離れた方がよく考えられます。'],
    note: '目に見えるものから入る安全な話題。後半で「自分の思考の場所」に話を移すと、雑談から人柄の話に自然に上がる。' },
  { id: 'kr-subway', cat: 'korea', icon: '🚇', title: '地下鉄で移動してみた',
    opener: "I took the subway from the hotel instead of a taxi. The signs are in English, the trains are on time, and it cost almost nothing.",
    openerJa: 'ホテルからタクシーではなく地下鉄を使いました。英語表示があり、時刻は正確で、料金はほとんどかからない。',
    follow: ["Do you commute by subway, or do you drive?",
             "Is there a line I should avoid at rush hour? I would like to survive this week."],
    followJa: ['あなたは地下鉄通勤ですか?それとも車?', 'ラッシュ時に避けるべき路線はありますか?今週を生き延びたいので。'],
    note: '「現地の交通を使ってみた」は相手の街への敬意になる。通勤の話は生活の話につながり、家族やスケジュールの話に自然に展開する。' },
  { id: 'jp-fermented', cat: 'japan', icon: '🍶', title: '日本の発酵食品',
    opener: "At a yeast meeting I should confess something — miso, soy sauce, sake, and mirin all sit on my kitchen shelf, and all of them are microbiology.",
    openerJa: '酵母の学会なので白状しますが — 味噌、醤油、日本酒、みりん、全部うちの台所にあって、全部が微生物学です。',
    follow: ["Is there a Korean fermented food that foreigners usually misunderstand?",
             "Have you ever visited a traditional brewery? I went to one and understood nothing, but the smell was unforgettable."],
    followJa: ['外国人が誤解しがちな韓国の発酵食品はありますか?', '伝統的な醸造所を訪ねたことはありますか?私は一度行って何も理解できませんでしたが、匂いは忘れられません。'],
    note: '酵母の学会では最強の共通言語。日韓どちらの食文化も下げず、「自分の台所」という個人的な入口から入るのが効く。' },
  { id: 'jp-sake-yeast', cat: 'japan', icon: '🏮', title: '清酒酵母の話',
    opener: "Japanese sake breweries use numbered yeast strains — kyokai number seven, number nine, and so on. Brewers argue about them the way we argue about lab strains.",
    openerJa: '日本の酒蔵は番号のついた酵母を使います。協会7号、9号、という具合に。醸造家はそれを、私たちが実験室株を議論するのと同じ熱量で語ります。',
    follow: ["Do Korean makgeolli producers have named strains like that?",
             "If someone made a beer with your favourite lab strain, would you drink it?"],
    followJa: ['韓国のマッコリの作り手にも、そういう名前のついた株はありますか?', 'もし誰かがあなたの好きな実験室株でビールを作ったら、飲みますか?'],
    note: '産業と基礎をつなぐ話題。最後の仮定の質問は、応用系の研究者にも基礎系にも同じくらい刺さる。' },
  { id: 'hm-bell', cat: 'humor', icon: '🔔', title: '時間切れのベル',
    opener: "My personal record is being cut off with four slides left. Now I design every talk so that the last slide is one I can skip without anyone noticing.",
    openerJa: '私の記録は、スライドを4枚残して打ち切られたことです。今は毎回、最後の1枚は誰にも気づかれずに飛ばせる構成にしています。',
    follow: ["What is your strategy when the chair starts making eye contact?",
             "Have you ever been the chair who had to stop someone? That looks harder."],
    followJa: ['座長が目で合図し始めたとき、あなたはどうしますか?', '誰かを止める側の座長をやったことは?あちらの方が難しそうです。'],
    note: '失敗談から入る定番。座長経験者は必ず語りたがるので、相手に主役を渡せる。発表直後・直前のどちらでも使える。' }
];
